...
### F-20. 디자인 스킨 ~~(구 컬러 테마)~~
- **[v2.14 변경]** 색상 테마 선택 기능은 **제거**되고 **디자인 스킨** 시스템으로 대체됨. 스킨은 형태(모서리·테두리·그림자·표면·폰트)와 강조색을 함께 정의하는 단일 커스터마이즈 축이다.
- **스킨 8종**: 기본(노션 웜)·뉴브루탈·클레이·미니멀 모노·아우로라 글래스·소프트 팝·슬릭 다크·페이퍼. 각 스킨은 라이트/다크 2벌 + 자연 모드(선택 시 라이트/다크 자동 전환)를 가진다.
- **설명**: `frontend/src/utils/skins.js`가 `--fd-sk-*` CSS 변수(모서리·테두리·그림자·레일·KPI 등)와 강조색을 주입. Zustand `themeStore`가 스킨 축을 관리(localStorage + 서버 `theme_prefs` 저장). `App.jsx` ConfigProvider의 AntD 표면/테두리 토큰(`colorBgContainer`·`colorBorder`·`lineWidth` 등)을 스킨값에 연결해 인라인 `token.*`을 쓰는 화면(메일·게시판·전자결재 등)까지 자동 반영.
- **모드**: 라이트/다크 토글(헤더)은 유지. 스킨 선택 시 해당 스킨의 자연 모드로 자동 전환되며, 이후 수동 토글 가능.
