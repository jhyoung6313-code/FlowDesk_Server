    ...
    >
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 4 }}>
            ...
            <Space size={4}>
              {isActive ? (
                ...
                >
                  <Tag color={sm.color} style={{ fontSize: 12, margin: 0, cursor: 'pointer' }}>
                    {sm.label} ▾
                  </Tag>
                </Dropdown>
              ) : (
                <Tag color={sm.color} style={{ fontSize: 12, margin: 0 }}>{sm.label}</Tag>
              )}
              {step.assignee && (
              ...
              {step.dueAt && (
                <Tag icon={<ClockCircleOutlined />} color="orange" style={{ fontSize: 12, margin: 0 }}>
              ...
              {isActive && (
                <Popconfirm title="이 항목을 삭제하시겠습니까?" okText="삭제" cancelText="취소"
                  ...
                  <Button type="text" size="small" danger icon={<DeleteOutlined style={{ fontSize: 12 }} />} style={{ padding: '0 2px' }} />
                </Popconfirm>
              )}
            </Space>
          </div>
          ...
          {/* SLA 경고 */}
          {slaWarn && (
            <Alert message={slaWarn} type="warning" showIcon banner style={{ marginTop: 4, fontSize: 12 }} />
          )}

          {/* 지침 */}
          {/* 상세 내용 */}
          <div style={{ marginTop: 6 }}>
            <Text type="secondary" style={{ fontSize: 12, display: 'block', marginBottom: 2 }}>상세 내용</Text>
            {isActive ? (
              <TextArea
                value={instrInput}
              ...
              <Space size={4} wrap>
                {step.status === 'pending' && (
                  <Button size="small" type="primary" onClick={() => handleAction('in_progress')} loading={actionLoading}
                    style={{ fontSize: 12, padding: '0 6px', height: 20, color: '#fff' }}>
                    시작
                  </Button>
                )}
                  ...
                  <>
                    {step.type === 'approval' ? (
                      ...
                      <Button size="small" type="primary" onClick={handleComplete} loading={actionLoading}
                        style={{ fontSize: 12, padding: '0 6px', height: 20 }}>
          ...
          {/* 분기 선택 결과 */}
          {step.decisionChosen && (
            <Tag color="purple" style={{ marginTop: 4, fontSize: 12 }}>선택: {step.decisionChosen}</Tag>
          )}
          ...
          <div style={{ marginTop: 8 }}>
            <Button
              ...
              style={{ fontSize: 12, color: '#888', padding: 0 }}
            ...
            <div style={{ marginTop: 6, paddingLeft: 4 }}>
              {checklists.map((item) => (
                <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                  ...
                  {isActive && (
                    <Button type="text" size="small" danger onClick={() => handleDeleteChecklistItem(item.id)} style={{ padding: 0, fontSize: 12 }}>✕</Button>
                  )}
                </div>
              ))}
              {isActive && (
        ...
        run.updates.map((u) => (
          <Card key={u.id} size="small" style={{ marginBottom: 8 }} bodyStyle={{ padding: '10px 14px' }}>
            <div style={{ display: 'flex', gap: 8 }}>
              <Avatar size="small" style={{ backgroundColor: u.creator?.avatarColor || '#1677ff', fontSize: 12, flexShrink: 0 }}>
              ...
              {(user?.role === 'admin' || u.createdBy === user?.id) && (
                <Popconfirm title="삭제?" onConfirm={() => handleDelete(u.id)}>
                  <Button type="text" size="small" danger style={{ fontSize: 12 }}>삭제</Button>
                </Popconfirm>
              )}
            </div>
          </Card>
        ...
        run.participants.map((p) => (
          <Card key={p.id} size="small" style={{ marginBottom: 8 }} bodyStyle={{ padding: '10px 14px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <Space>
                <Avatar size="small" style={{ backgroundColor: p.user?.avatarColor || '#1677ff', fontSize: 12 }}>
                  {p.user?.displayName?.slice(0, 1)}
                </Avatar>
                <Text>{p.user?.displayName}</Text>
                <Tag color={ROLE_COLORS[p.role] || 'default'} style={{ fontSize: 12 }}>
            ...
            return (
              <div key={ph.id}>
                <Divider orientation="left" orientationMargin={0} style={{ fontSize: 19.5, margin: '12px 0 8px' }}>
                  ...
                  {ph.name}
                  <span style={{ marginLeft: 8, fontSize: 12, color: '#aaa', fontWeight: 400 }}>
